...
interface IssueCardFieldsProps {
  issue: Issue;
}

export const IssueCardFields = ({ issue }: IssueCardFieldsProps) => {
  ...
  return boardCardFieldEntries.map((fieldEntry, index) => {
    if (fieldEntry.kind === 'system') {
      ...
      if (fieldEntry.systemFieldId === 'startDate') {
        return issue.startDate && (
          <span key={fieldEntry.id} className="text-muted-foreground text-xs">
